...
export function CheckoutView() {
  ...
  // Traemos TODO del store (aún no lo vaciamos)
  const { orderData, items: cartItems, clearCart } = useCartStore();
  
  // Traemos la lista de adicionales de la BD para armar el recibo
  ...
  const subtotal = cartItems.reduce((acc, item) => {
    const extrasTotal = Object.entries(item.adicionales || {}).reduce((sum, [eId, q]) => {
      // Buscamos el precio del extra real por su _id
  ...
  const discount = orderData.couponCode ? subtotal * 0.10 : 0; // Ajustá tu % de descuento acá
  const total = subtotal - discount;

  const finalizarPedido = () => {
    clearCart(); // Recién acá limpiamos la memoria
    router.push('/');
  };
  ...
  const handleWhatsApp = () => {
    ...
    // Armamos la lista de productos y adicionales para WhatsApp
    const listaProductosText = cartItems.map(item => {
      let textoItem = `- ${item.quantity}x ${item.title} ($${item.price * item.quantity})`;
      
      const extras = Object.entries(item.adicionales || {}).filter(([_, q]) => (q as number) > 0);
      if (extras.length > 0) {
    ...
    finalizarPedido(); // Vaciamos y redirigimos
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col relative pb-10">
      ...
      <main className="flex-1 flex flex-col items-center px-4 pt-20 max-w-lg mx-auto w-full">
        ...
        <h1 className="font-['Bebas_Neue'] text-4xl md:text-5xl text-foreground mb-1 text-center tracking-wide">
          ¡PEDIDO ENVIADO!
        </h1>
        ...
        <div className="bg-secondary/50 rounded-xl p-4 mb-6 w-full text-center border border-border">
          <p className="text-sm text-foreground font-medium">
            Avisanos por WhatsApp para coordinar el pago en <strong>{orderData.paymentMethod}</strong> y el {orderData.deliveryType === 'delivery' ? 'envío' : 'retiro'}.
          </p>
        </div>
        ...
        <button 
          onClick={finalizarPedido}
          className="mt-5 text-muted-foreground font-bold hover:text-primary transition-colors cursor-pointer"
        >
          Volver al Menú
        </button>
      </main>
